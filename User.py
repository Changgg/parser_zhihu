...
class User(object):
    ...
    def __init__(self, cookies: dict):
        ...
        session.headers['user-agent'] = faker.Faker().chrome()
...
class Users(object):
    ...
    def login(self, username, password):
        ...
        status = self.session.post(Utils.PHONE_LOGIN, headers=Utils.Default_Headers, data=data)
        print(status.json()['msg'])

        if status.status_code == 200:
            u = User(cookies=requests.utils.dict_from_cookiejar(self.session.cookies))
            self.Users.append(u)
            return True
        else:
            pass
    ...
    def archieve_captcha(self):
        c = self.session.get(Utils.CAPTCHA_URL, headers=Utils.Default_Headers)
        print(Utils.CAPTCHA_URL)
    ...
    def isValid(self):
        cookieJar = requests.utils.cookiejar_from_dict(Utils.COOKIE)
    ...
    def _find_xsrf(self):
        c = BeautifulSoup(self.session.get(Utils.URL_BASE, headers=Utils.Default_Headers).content, 'lxml')
        xsrf = c.find('input', {'name': '_xsrf'})
        return xsrf

    def archieve_userinfo(self, userid):
        print('{}{}/answers'.format(Utils.PERSOINFO_API, userid))
        r = self.session.get(url='{}{}/answers'.format(Utils.PERSOINFO_API, userid), headers=Utils.Default_Headers)
    ...
    def init_followee_url(self, userid, ftype):
        ...
        url = Utils.MEMBER_API + '/' + userid + '/' + ftype
